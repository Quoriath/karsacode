...
export function AddProviderInstanceDialog({ open, onOpenChange }: AddProviderInstanceDialogProps) {
  ...
  const customAgentApiKeyEnvVar =
    readProviderConfigString(configDraft, "apiKeyEnvVar") || "OPENAI_API_KEY";
  ...
  useEffect(() => {
    if (!open || driver !== CUSTOM_AGENT_DRIVER_KIND) return;
    setEnvironment((current) => {
      if (current.length === 0) {
        return [{ name: customAgentApiKeyEnvVar, value: "", sensitive: true }];
      }
      const [only] = current;
      if (
        current.length === 1 &&
        ...
        only.value.length === 0 &&
        only.sensitive &&
        only.name !== customAgentApiKeyEnvVar
      ) {
        return [{ ...only, name: customAgentApiKeyEnvVar }];
      }
      return current;
    });
  }, [customAgentApiKeyEnvVar, driver, open]);
  ...
  const handleSave = useCallback(() => {
    ...
    const config =
      driver === CUSTOM_AGENT_DRIVER_KIND && customAgentApiKey.trim().length > 0
        ? { ...(configByDriver[driver] ?? {}), apiKey: customAgentApiKey.trim() }
    ...
    const cleanedEnvironment = (
      driver === CUSTOM_AGENT_DRIVER_KIND
        ? upsertSensitiveProviderEnvironmentValue(
            environment,
            customAgentApiKeyEnvVar,
            customAgentApiKey.trim(),
          )
        : environment
                ...
                {driver === CUSTOM_AGENT_DRIVER_KIND ? (
                  <label className="grid gap-1.5">
                    ...
                    <Input
                      ...
                      onChange={(event) => setCustomAgentApiKey(event.target.value)}
                    />
                    <span className="text-[11px] text-muted-foreground">
                      Stored as sensitive environment variable{" "}
                      <code>{customAgentApiKeyEnvVar}</code>.
                    </span>
                  </label>
                ) : null}
