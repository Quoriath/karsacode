...
export function readProviderConfigStringListInput(config: unknown, key: string): string {
  if (config === null || typeof config !== "object") return "";
  const value = (config as Record<string, unknown>)[key];
...
export function readProviderConfigStringRecordInput(config: unknown, key: string): string {
  if (config === null || typeof config !== "object") return "";
  const value = (config as Record<string, unknown>)[key];
...
export function nextProviderConfigWithFieldValue(
  ...
  if (typeof value === "number") {
    if (field.clearWhenEmpty === "omit" && !Number.isFinite(value)) {
      delete base[field.key];
    } else {
      base[field.key] = value;
    }
  ...
  if (Array.isArray(value)) {
    ...
    if (field.clearWhenEmpty === "omit" && entries.length === 0) {
      delete base[field.key];
    } else {
      base[field.key] = entries;
    }
    return Object.keys(base).length > 0 ? base : undefined;
  }

  if (typeof value === "object") {
    if (field.clearWhenEmpty === "omit" && Object.keys(value).length === 0) {
      delete base[field.key];
    } else {
      base[field.key] = { ...value };
    }
...
function ProviderSettingsFieldRow({
        ...
        <label htmlFor={inputId} className={cn(variant === "card" && "block")}>
          ...
          {variant === "card" ? (
            <DraftInput
              ...
              className="mt-1.5"
              type="number"
              value={readProviderConfigNumberInput(value, field.key)}
              onCommit={(next) => {
                ...
                onChange(
                  nextProviderConfigWithFieldValue(
                    value,
                    field,
                    trimmed.length === 0 ? "" : Number(trimmed),
                  ),
                );
              }}
    ...
    return (
      <FieldFrame variant={variant}>
        <label htmlFor={inputId} className={cn(variant === "card" && "block")}>
          ...
          <Textarea
            id={inputId}
            className={cn(variant === "card" && "mt-1.5", variant === "dialog" && "bg-background")}
            value={readProviderConfigStringListInput(value, field.key)}
            onChange={(event) =>
              onChange(
                nextProviderConfigWithFieldValue(value, field, event.target.value.split(/\r?\n/u)),
              )
            }
            placeholder={field.placeholder}
            spellCheck={false}
  ...
  if (field.control === "json-record") {
    const commitJsonRecord = (next: string) => {
      const raw = next.trim();
      if (raw.length === 0) {
      ...
      <FieldFrame variant={variant}>
        <label htmlFor={inputId} className={cn(variant === "card" && "block")}>
          ...
          <DraftInput
            id={inputId}
            className={cn(variant === "card" && "mt-1.5", variant === "dialog" && "bg-background")}
            value={readProviderConfigStringRecordInput(value, field.key)}
            onCommit={commitJsonRecord}
            placeholder={field.placeholder}
            spellCheck={false}
          />
          {description}
