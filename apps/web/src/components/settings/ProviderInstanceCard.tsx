...
const CUSTOM_AGENT_DRIVER_KIND = ProviderDriverKind.make("customAgent");
const DEFAULT_CUSTOM_AGENT_API_KEY_ENV_VAR = "OPENAI_API_KEY";
...
function readCustomAgentApiKeyEnvVar(config: unknown): string {
...
export function upsertSensitiveProviderEnvironmentValue(
  environment: ReadonlyArray<ProviderInstanceEnvironmentVariable> | undefined,
  name: string,
  value: string,
): ReadonlyArray<ProviderInstanceEnvironmentVariable> {
  if (value.length === 0) return environment ?? [];
  const current = environment ?? [];
  const index = current.findIndex((variable) => variable.name === name);
  if (index === -1) {
    return [...current, { name, value, sensitive: true }];
  }
  return current.map((variable, rowIndex) =>
    rowIndex === index
      ? {
          ...variable,
          name,
...
export function renameCustomAgentApiKeyEnvironmentVariable(input: {
  ...
  if (
    ...
    environment.some((row) => row.name === input.nextName)
  ) {
    return environment;
  }
  const exactIndex = environment.findIndex(
    (row) => row.name === input.previousName && row.sensitive,
  );
  const fallbackIndex =
    exactIndex === -1 && environment.length === 1 && environment[0]?.sensitive ? 0 : exactIndex;
  if (fallbackIndex === -1) return environment;
  return environment.map((row, index) =>
    index === fallbackIndex ? { ...row, name: input.nextName } : row,
  );
}
...
export function ProviderInstanceCard({
  ...
  const updateConfig = (nextConfig: Record<string, unknown> | undefined) => {
    ...
    const nextInstance =
      nextConfig !== undefined
        ...
        : (rest as ProviderInstanceConfig);
    if (instance.driver !== CUSTOM_AGENT_DRIVER_KIND) {
      onUpdate(nextInstance);
      return;
    }
    const previousName = readCustomAgentApiKeyEnvVar(instance.config);
    const nextName = readCustomAgentApiKeyEnvVar(nextConfig);
    const environment = renameCustomAgentApiKeyEnvironmentVariable({
      environment: instance.environment,
      previousName,
      nextName,
    }).filter((variable) => variable.name.trim().length > 0);
    const { environment: _environment, ...withoutEnvironment } = nextInstance;
    onUpdate(
      environment.length > 0
        ? ({ ...withoutEnvironment, environment } as ProviderInstanceConfig)
        : (withoutEnvironment as ProviderInstanceConfig),
    );
  };
  ...
  const updateCustomAgentApiKey = (value: string) => {
    ...
    const environment = upsertSensitiveProviderEnvironmentValue(
      instance.environment,
      readCustomAgentApiKeyEnvVar(instance.config),
      apiKey,
    );
    onUpdate({
      ...instance,
      environment,
      config: nextCustomAgentConfigWithApiKey(instance.config, apiKey),
              ...
              <div className="border-t border-border/60 px-4 py-3 sm:px-5">
                <label htmlFor={`provider-instance-${instanceId}-api-key`} className="block">
                  ...
                  <DraftInput
                    ...
                    autoComplete="off"
                    value=""
                    onCommit={updateCustomAgentApiKey}
                    placeholder={
                      instance.environment?.some(
                        (variable) =>
                          variable.name === readCustomAgentApiKeyEnvVar(instance.config) &&
                          variable.sensitive &&
                  ...
                  />
                  <span className="mt-1 block text-xs text-muted-foreground">
                    Stored as sensitive environment variable{" "}
                    <code>{readCustomAgentApiKeyEnvVar(instance.config)}</code>.
                  </span>
                </label>
              </div>
            ) : null}
