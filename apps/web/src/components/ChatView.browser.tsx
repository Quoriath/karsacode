...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.karsacode-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
...
describe("ChatView timeline estimator parity (full app)", () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              KARSACODE_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(openRequest?.env?.KARSACODE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("karsacode:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              KARSACODE_PROJECT_ROOT: "/repo/project",
              KARSACODE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/pingdotgg/karsacode/pull/1359",
              baseBranch: "main",
          ...
          expect(dispatchRequest).toMatchObject({
            ...
            bootstrap: {
              ...
              prepareWorktree: {
                ...
                baseBranch: "main",
                branch: expect.stringMatching(/^karsacode\/[0-9a-f]{8}$/),
              },
              runSetupScript: true,
            },
          });
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    ...
    try {
      ...
      localStorage.removeItem("karsacode:client-settings:v1");
      await mounted.cleanup();
    }
  });
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: KarsaCode Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
