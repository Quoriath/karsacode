...
process.env.KARSACODE_CURSOR_ENABLED = "1";
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `karsacode_codex_missing_${process.pid}_${Date.now()}`;
      ...
      live("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          const firstMissing = `karsacode_codex_first_${process.pid}_${Date.now()}`;
          const secondMissing = `karsacode_codex_second_${process.pid}_${Date.now()}`;
          const serverSettings = yield* makeMutableServerSettingsService(
            Schema.decodeSync(ServerSettings)(
              deepMerge(DEFAULT_SERVER_SETTINGS, {
                providers: {
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("runs Claude status probes with the configured Claude HOME", () => {
        const claudeHome = "/tmp/karsacode-claude-home";
