...
const APP_USER_MODEL_ID = isDevelopment ? "com.t3tools.karsacode.dev" : "com.t3tools.karsacode";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "karsacode-dev.desktop" : "karsacode.desktop";
const LINUX_WM_CLASS = isDevelopment ? "karsacode-dev" : "karsacode";
const USER_DATA_DIR_NAME = isDevelopment ? "karsacode-dev" : "karsacode";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "KarsaCode (Dev)" : "KarsaCode (Alpha)";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("KarsaCode failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `KarsaCode ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/KarsaCode (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`karsacode`). If the legacy
