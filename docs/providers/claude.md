...
This guide is for people who want to use more than one Claude setup in KarsaCode.
...
In KarsaCode Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means KarsaCode uses your normal home directory.
...
Then add another Claude provider in KarsaCode:
...
KarsaCode only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so KarsaCode keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in KarsaCode Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. KarsaCode stores the value as a server secret and does not
...
KarsaCode does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in KarsaCode.
