...
This guide is for people who want to use more than one Codex account in KarsaCode.
...
In KarsaCode Settings, name it something obvious:
...
In KarsaCode Settings, add another Codex provider:
...
KarsaCode shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, KarsaCode treats it
