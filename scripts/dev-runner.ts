...
export const DEFAULT_KARSACODE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".karsacode"),
);
...
const MODE_ARGS = {
  ...
  "dev:server": ["run", "dev", "--filter=karsacode"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("KARSACODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("KARSACODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid KARSACODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `KARSACODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric KARSACODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed KARSACODE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_KARSACODE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly karsacodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(karsacodeHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      KARSACODE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.KARSACODE_PORT = String(serverPort);
      ...
      delete output.KARSACODE_MODE;
      delete output.KARSACODE_NO_BROWSER;
      delete output.KARSACODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.KARSACODE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.KARSACODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.KARSACODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.KARSACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.KARSACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.KARSACODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.KARSACODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.KARSACODE_MODE = "web";
      delete output.KARSACODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read KARSACODE_PORT_OFFSET/KARSACODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const env = yield* createDevRunnerEnv({
      ...
      serverOffset,
      webOffset,
      karsacodeHome: input.karsacodeHome,
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.KARSACODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.KARSACODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  karsacodeHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all KarsaCode data (equivalent to KARSACODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("KARSACODE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to KARSACODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("KARSACODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to KARSACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("KARSACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to KARSACODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("KARSACODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to KARSACODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("KARSACODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to KARSACODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("KARSACODE_PORT")),
  ),
